...
1. Extract `ip` from the `x_forwarded_for` attribute
   - use the third-to-last value (since the second-to-last value is a forwarding rule IP added by
     Google load balancer, and the last value is a Google load balancer IP added by nginx)
