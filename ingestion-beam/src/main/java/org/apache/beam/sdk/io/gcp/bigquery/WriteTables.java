...
class WriteTables<DestinationT> extends
  ...
  private class WriteTablesDoFn
    ...
    private class PendingJobData {
      ...
      public PendingJobData(BoundedWindow window, BigQueryHelpers.PendingJob retryJob,
          List<String> partitionFiles, TableDestination tableDestination,
    ...
    public void processElement(ProcessContext c, BoundedWindow window) throws Exception {
      ...
      if (firstPaneCreateDisposition == CreateDisposition.CREATE_NEVER) {
        ...
        tableSchema = BigQueryHelpers.fromJsonString(jsonSchemas.get(destination),
            TableSchema.class);
      } else {
        tableSchema = dynamicDestinations.getSchema(destination);
        checkArgument(tableSchema != null,
            ...
            CreateDisposition.CREATE_NEVER, firstPaneCreateDisposition, dynamicDestinations,
            destination);
        jsonSchemas.put(destination, BigQueryHelpers.toJsonString(tableSchema));
      }

      TableDestination tableDestination = dynamicDestinations.getTable(destination);
      checkArgument(tableDestination != null, "DynamicDestinations.getTable() may not return null, "
          + "but %s returned null for destination %s", dynamicDestinations, destination);
      ...
      String jobIdPrefix = BigQueryHelpers.createJobId(c.sideInput(loadJobIdPrefixView),
          tableDestination, partition, c.pane().getIndex());
      ...
      BigQueryHelpers.PendingJob retryJob = startLoad(
          bqServices.getJobService(c.getPipelineOptions().as(BigQueryOptions.class)),
          bqServices.getDatasetService(c.getPipelineOptions().as(BigQueryOptions.class)),
          jobIdPrefix, tableReference, tableDestination.getTimePartitioning(), tableSchema,
          partitionFiles, writeDisposition, createDisposition);
      pendingJobs.add(
    ...
    public void finishBundle(FinishBundleContext c) throws Exception {
      DatasetService datasetService = bqServices
      ...
      for (PendingJobData pendingJob : pendingJobs) {
        jobManager = jobManager.addPendingJob(pendingJob.retryJob,
            ...
            j -> {
              try {
                ...
                c.output(mainOutputTag,
                    KV.of(pendingJob.tableDestination,
                ...
                for (String file : pendingJob.partitionFiles) {
                  c.output(temporaryFilesTag, file, pendingJob.window.maxTimestamp(),
                      pendingJob.window);
                }
                return null;
  ...
  public WriteTables(boolean tempTable, BigQueryServices bqServices,
      PCollectionView<String> loadJobIdPrefixView, WriteDisposition writeDisposition,
      CreateDisposition createDisposition, List<PCollectionView<?>> sideInputs,
      DynamicDestinations<?, DestinationT> dynamicDestinations,
      @Nullable ValueProvider<String> loadJobProjectId, int maxRetryJobs,
      boolean ignoreUnknownValues, String kmsKey) {
  ...
  public PCollection<KV<TableDestination, String>> expand(
    ...
    PCollectionTuple writeTablesOutputs = input
        .apply(ParDo.of(new WriteTablesDoFn()).withSideInputs(sideInputs)
    ...
    writeTablesOutputs.get(temporaryFilesTag).setCoder(StringUtf8Coder.of())
        .apply(WithKeys.of((Void) null)).setCoder(KvCoder.of(VoidCoder.of(), StringUtf8Coder.of()))
  ...
  private PendingJob startLoad(JobService jobService, DatasetService datasetService,
      String jobIdPrefix, TableReference ref, TimePartitioning timePartitioning,
      @Nullable TableSchema schema, List<String> gcsUris, WriteDisposition writeDisposition,
      CreateDisposition createDisposition) {
    JobConfigurationLoad loadConfig = new JobConfigurationLoad().setDestinationTable(ref)
        .setSchema(schema).setSourceUris(gcsUris).setWriteDisposition(writeDisposition.name())
    ...
    // !!!!!!!
    // This line is a custom patch to support Mozilla's use case until the Beam project
    // merges in support for writing to clustered tables: https://github.com/apache/beam/pull/7061
    // This line assumes that ALL TABLES we load into are clustered on submission_timestamp;
    // with this patch in place, attempts to write to tables with any other clustering configuration
    // will fail.
    // !!!!!!!
    loadConfig.setClustering(new Clustering().setFields(ImmutableList.of("submission_timestamp")));
    ...
    PendingJob retryJob = new PendingJob(
        // Function to load the data.
        jobId -> {
          JobReference jobRef = new JobReference().setProjectId(projectId)
              .setJobId(jobId.getJobId()).setLocation(bqLocation);
          LOG.info("Loading {} files into {} using job {}, job id iteration {}", gcsUris.size(),
              ref, jobRef, jobId.getRetryIndex());
          try {
