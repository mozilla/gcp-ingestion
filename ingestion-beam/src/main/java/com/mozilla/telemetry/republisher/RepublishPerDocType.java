  ...
  public PDone expand(PCollection<PubsubMessage> input) {
    ...
    for (int i = 0; i < numDestinations; i++) {
      ...
      opts.setOutput(StaticValueProvider.of(baseOptions.getPerDocTypeDestination()
          .replace("${document_namespace}", destination.getDisplayNamespace())
          .replace("${document_type}", destination.getDisplayDocType())));

      String name = String.join("_", "republish", destination.getDisplayNamespace(),
          destination.getDisplayDocType());
  ...
  private static class Destination implements Serializable {
    ...
    public boolean matches(String namespaceToMatch, String docTypeToMatch) {
      return namespace.equals(namespaceToMatch)
          && (docType.equals("*") || docType.equals(docTypeToMatch));
    }
